<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('admin.gallery.edit', $gallery) }}
    @endsection
        ...
        <form action="{{ route('admin.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-9">
                        <input type="text" name="title" class="form-control form-control-lg @error('title') is-invalid @enderror" 
                            placeholder="Enter gallery item title" value="{{ old('title', $gallery->title) }}" required />
                        @error('title')
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-9">
                        <textarea name="description" class="form-control form-control-lg @error('description') is-invalid @enderror" 
                            rows="4" placeholder="Enter description">{{ old('description', $gallery->description) }}</textarea>
                        @error('description')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-9">
                        ...
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                        <small class="d-block mt-2 text-muted">Leave empty to keep current image. Allowed formats: JPEG, PNG, JPG, GIF, WebP (Max: 2MB)</small>
                        
                        <!-- New Image Preview -->
                        <div id="imagePreview" class="mt-3" style="display: none;">
                            <img id="previewImg" src="" alt="Preview" class="img-fluid rounded" style="max-width: 300px; max-height: 300px;">
                            <small class="d-block mt-2 text-muted">New Image Preview</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-9">
                        <input type="number" name="order" class="form-control form-control-lg @error('order') is-invalid @enderror" 
                            placeholder="0" value="{{ old('order', $gallery->order) }}" min="0" />
                        @error('order')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-lg-9">
                        <div class="form-check form-check-custom form-check-solid">
                            <input class="form-check-input" type="checkbox" name="is_active" value="1" 
                                id="is_active" {{ $gallery->is_active ? 'checked' : '' }} />
                ...
                <!-- Meta Info -->
                <div class="row mb-6">
                    <label class="col-lg-3 col-form-label fw-semibold fs-6">Additional Info</label>
                    <div class="col-lg-9">
                        <div class="row">
                            <div class="col-md-6 mb-3 mb-md-0">
                                <small class="text-muted d-block">Created: {{ $gallery->created_at->format('M d, Y H:i') }}</small>
                                <small class="text-muted d-block">Updated: {{ $gallery->updated_at->format('M d, Y H:i') }}</small>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-footer d-flex justify-content-end">
                ...
                <button type="submit" class="btn btn-primary">
                    <span class="indicator-label">Update Gallery Item</span>
                </button>
            </div>
        </form>
    </div>
